...
public class SimpleEngine implements TestEngine {
        ...
        for (TestDescriptor test : desc.getChildren()) {
            ...
            if (test instanceof ScenarioTestDescriptor) {
                ...
                try {
                    ...
                    request.getEngineExecutionListener().executionFinished(test, TestExecutionResult.failed(e));
                }
                ...
                try {
                    ...
                    DependencyService service = (DependencyService) dependency.getJavaMethod().invoke(arena);
    ...
    private void executeArena(ExecutionRequest request, ScenarioTestDescriptor stest) throws InstantiationException, IllegalAccessException {
        ...
        try {
            ...
            request.getEngineExecutionListener().executionFinished(stest, TestExecutionResult.failed(e));
        }
    }

    private void handleDependency(ExecutionRequest request, ScenarioDependencyDescriptor test, DependencyService service) {
        try {
            service.init();
            service.start();
            service.stop();
